...
private:
    ...
    struct Block {
        ...
        void push_front(const T& value) {
            ...
            memmove(data + 1, data, sizeof(T) * size);
            new (data) T(value);
            ++size;
        }
...
public:
    ...
    iterator insert(iterator pos, const T &value) {
        ...
        // General case: split the block if needed
        Block* target_block = pos.block;
        size_t target_index = pos.index;

        if (target_block->size < target_block->capacity) {
            // Room in the block
            memmove(target_block->data + target_index + 1,
                    target_block->data + target_index,
                    sizeof(T) * (target_block->size - target_index));
        ...
        if (target_index <= split_point) {
            ...
            memmove(new_block->data + new_index + 1,
                    new_block->data + new_index,
                    sizeof(T) * (new_block->size - new_index));
    ...
    iterator erase(iterator pos) {
        ...
        memmove(target_block->data + target_index,
                target_block->data + target_index + 1,
                sizeof(T) * (target_block->size - target_index - 1));
        --target_block->size;
        --total_size;
